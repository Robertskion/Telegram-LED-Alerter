...
def get_args():
    parser = argparse.ArgumentParser(
        description="Telegram LED strip alerting script - GPIO work in BCM mode",
        usage='use "./%(prog)s --help" for more information')
    parser.add_argument(
        '-l', '--ledsGPIO', type=str, help='List of GPIO pins used by LED strip channels (3 in RGB), in order to have '
                                           'the beautiful effect. Values sparated by commas.',
        required=True, nargs='+')
    parser.add_argument(
            ...
            try:
                ...
                while input_state == 1:
                    ...
                    for y in range(len(ledsGPIO)):
                        LEDsCh[y].start(0)
                        time.sleep(blinkSpeed)
                        LEDsCh[y - 1].start(100)
                        if y is not max(range(len(ledsGPIO))):
                            time.sleep(blinkSpeed)
...
for led in range(len(ledsGPIO)):
    ...
    LEDsCh.append(GPIO.PWM(ledsGPIO[led], 100))
    LEDsCh[led].start(0)
